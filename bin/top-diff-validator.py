...
def run_top_diff_analysis(input_dir, output_dir, date, effect, nratios, frac, corr):
    ...
    plt.savefig(os.path.join(figure_dir, f"synthetic-top-degs-ml-prediction-roc-curve-plots-eff-{effect}-ndiff-{nratios}-frac-{frac}-corr-{corr}-{date}.svg"), 
                format='svg', 
                bbox_inches='tight')
...
def main():
    ...
    output_path = os.path.join(args.output_dir, f"top-diff-synthetic-validation-eff-{args.effect}-ndiff-{args.nratios}-frac-{args.frac}-corr-{args.corr}-{args.date}.tsv")
    if not os.path.exists(output_path):
        print(f"Effect Size: {args.effect} N Ratios: {args.nratios} Response Fraction: {args.frac} Correlation thresh: {args.corr}")
        stats = run_top_diff_analysis(args.input_dir, args.output_dir, args.date, args.effect, args.nratios, args.frac, args.corr)
